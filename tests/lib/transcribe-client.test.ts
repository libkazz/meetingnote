...
import * as transcribe from '../../src/lib/api/transcribe-client'
...
describe('transcribe-client', () => {
  ...
  it('uses the proxy URL when USE_PROXY is true', async () => {
    readEnvSpy.mockImplementation((key: string) => {
      ...
      if (key === 'N8N_TRANSCRIBE_URL') return undefined
      return undefined
    })
  ...
  it('throws a formatted error on non-ok responses', async () => {
    ...
    await expect(transcribe.transcribeAudio(new Blob(['x']))).rejects.toThrow(
  ...
  it('maps a timeout DOMException to a friendly message', async () => {
    ...
    await expect(transcribe.transcribeAudio(new Blob(['x']), { timeoutMs: 5 })).rejects.toThrow(
